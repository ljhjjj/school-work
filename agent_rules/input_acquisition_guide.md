...
## 4. Manifest 변경 허용 범위
manifest 자동 생성 및 상태 변경은 `task_type`이 `new_note` 또는 `update_note`로 확정된 경우에만 수행한다.

다음 요청에서는 manifest를 생성하거나 상태를 변경하지 않는다. 이 경우 `inbox/`는 read-only 입력 영역으로 취급하고, 필요한 변경은 제안만 보고한다.
- `review`, `summary`
- `dry_run`
- 작업 계획 작성 또는 `planning_only`
  - manifest 상태 변경은 수행하지 않는다.
  - 단, 사용자가 명시적으로 PLAN 파일 생성/갱신을 요청한 경우 PLAN 파일만 생성/갱신할 수 있다.
  - 본문 `.md` 작성은 수행하지 않는다.
- 규칙 파일 검토
- 질문 답변
...
- 사용자가 명시적으로 문서 작성 작업을 요청했고, `task_type`이 `new_note` 또는 `update_note`로 확정되었으며, 작업 폴더에 `input_manifest.json`이 없으며, 폴더명과 파일명으로 과목/주차/차시/파일 역할을 추론할 수 있으면 Codex는 `input_manifest.json`을 자동 생성할 수 있다.
...
- `review`, `dry_run`, `planning_only`, `summary`에서는 manifest를 자동 생성하지 않는다.


## 8. 완료 후 처리
- `task_type`이 `new_note` 또는 `update_note`인 실제 문서 작성/수정 작업이 정상 완료된 경우에만 manifest 상태를 `completed`로 바꾼다.
- `review`, `dry_run`, `planning_only`, `summary`에서는 manifest 상태를 변경하지 않는다.
...
4. manifest 최상위 `task_priority`가 없고 `status: pending`인 작업이 여러 개면, manifest에 명시된 `subject`의 알파벳/가나다 순으로 처리한다.
5. `subject`도 없으면 `input_root` 경로의 문자열 순서로 처리한다.
6. 최신 수정 시간이 가장 오래된 작업을 자동으로 우선하지 않는다.
7. 처리할 manifest 또는 입력 세트를 하나로 정할 수 없으면 사용자에게 한 번만 질문한다.

## 11. 상태 전이 규칙
- 상태 전이는 `task_type`이 `new_note` 또는 `update_note`인 실제 문서 작성/수정 작업에만 적용한다.
...
- `pending` → `in_progress` → `completed` 전이는 `task_type`이 `new_note` 또는 `update_note`인 경우에만 수행한다.
- `review`, `dry_run`, `planning_only`, `summary`에서는 위 상태 전이를 수행하지 않는다.
