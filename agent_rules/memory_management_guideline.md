...
2. **기본 연속 실행 원칙 (Continuous Execution by Default):** 특별한 블로커나 시스템 에러가 없는 한, 사용자의 개입 없이 계획된 목차의 작성을 끝까지 연속 실행합니다.
...
5. **TROUBLESHOOT (데이터 누락 및 시스템 예외 대기):** 
   - **[데이터 이슈]:** 심각한 데이터 누락 시 PLAN 파일에 이슈를 기록하고 사용자에게 캡처본을 요청합니다. 단, 재요청은 **최대 2회**로 제한하며, PLAN 파일에 시도 횟수를 기록합니다(예: `- *Retry Count:* 1/2`). 실패 시 `> ⚠️ [수식/내용 판독 불가: 수동 입력 필요]` 태그를 남기고 넘어갑니다.
