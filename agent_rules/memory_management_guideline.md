...
1. **일급 객체화 및 격리 원칙:** 단기 기억력 한계 극복 및 병렬 작업 충돌 방지를 위해, 계획 파일은 반드시 각 과목 폴더 내에 **차시 단위로 분리**하여 `[과목명]_[주차]-[차시]_PLAN.md` 형태로 저장합니다. (예: `고체역학/고체역학_3-1_PLAN.md`)
2. **기본 연속 실행 원칙 (Continuous Execution by Default):** 특별한 블로커나 시스템 에러가 없는 한, 사용자의 개입 없이 계획된 목차의 작성을 끝까지 연속 진행합니다. 

## 2. 작업 루틴 (Execution Loop)
새로운 문서 정리 요구사항을 받거나 작업을 재개할 때 다음 프로세스를 엄격히 따르세요:

1. **READ & INIT (읽기 및 초기화):** 
   - 셸 명령어(PowerShell: `Get-Content -Raw`, CMD: `type`, Bash: `cat`)로 **타겟 PLAN 파일(`[과목명]/[과목명]_[주차]-[차시]_PLAN.md`)**을 찾아 읽습니다. 파일이 없으면 폴더와 함께 새로 생성합니다.
...
5. **TROUBLESHOOT (데이터 누락 및 시스템 예외 대기):** 
   - **[데이터 이슈]:** 심각한 데이터 누락 시 PLAN 파일에 이슈를 기록하고 사용자에게 캡처본을 요청합니다. 단, 재요청은 **최대 2회**로 제한하며, 실패 시 `> ⚠️ [수식/내용 판독 불가: 수동 입력 필요]` 태그를 남기고 강제로 이어서 진행합니다.
   - **[시스템/실행 이슈]:** 토큰 초과, 툴 에러, 세션 종료 등으로 자율 연속 실행이 불가능해진 경우 무리하게 진행하지 마세요. 현재까지의 완료 범위를 `_PLAN.md`에 기록하고, `### ⚠️ 실행 이슈` 섹션에 실패 원인과 다음 재개 지점을 명확히 남긴 뒤 대기합니다.
...
- [ ] 3. 압축기 및 펌프 파트 
  - [x] *Blocker 해결:* 펌프 위치 에너지 사용자 제공 이미지 판독 완료.
  - [ ] *본문 작성:* 판독 내용 마크다운 변환 대기.

#### ⚠️ 실행 이슈 (Execution Issues)
- **발생일시:** 2026-04-28
- **이슈:** 세션 토큰 초과 및 툴 타임아웃 발생.
