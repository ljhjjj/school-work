...
- `lecture_slide.pdf`만 제공되어 role이 `primary_scope_anchor`이고 scope가 `entire_pdf`인 경우에는 다음 항목을 필수 검증 대상으로 삼는다.
...
- 구조 검증이 통과해도 입력 커버리지 검증이 미완료이면 작업 완료로 판정하지 않는다.
- PLAN 또는 최종 리포트에는 `구조 검증 결과`와 `입력 커버리지 검증 결과`를 분리해 기록한다.
